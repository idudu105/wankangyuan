...
public interface ProjectDao {
	 ...
	 * @return 我加入的项目列表
	 */
	@Select("select project.* from project , project_user " + "where project.id=project_user.project_id "
			+ "and project_user.user_id=#{user_id} " + "and project.p_name like '%${searchWord}%' "
	 ...
	 */
	@Select("select project.* from project , project_user " + "where project.id=project_user.project_id "
			+ "and project_user.user_id=#{user_id} " + "order by project.id desc")
