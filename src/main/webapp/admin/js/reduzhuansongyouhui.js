    ...
    if(id){
        $.ajax({
            ...
            //是否显示修改
                ...
                //全场折扣
                $(".peisong").val(data.rule);
                //规则设定
                if(data.order_limit_type==1){
                    $(".meitian ").attr("checked","true");
                    $(".meitian ").parent().addClass("checked")
                    $(".every").val(data.order_limit);
                }else{
                    $(".huodong ").attr("checked","true");
                    $(".huodong ").parent().addClass("checked")
                    $(".during").val(data.order_limit);
    ...
    $(".check").click(function(){
        ...
        if($(this).attr('id')==1){
            order_limit_type=1;
            $(".during").val("")
            order_limit =  $(".every").val();
            // order_limitvar =$(this).val();
        }else if($(this).attr('id')==2){
            order_limit_type=2;
            // order_limitvar = $(".during").val()
            $(".every").val("")
            order_limit =  $(".during").val();
        }
    });
    ...
    $(".baocun").click(function(){
        ...
        if(startdate>endate){
            ...
            if(endate<date){
                ...
                if(id){
                    //order_limit_type 的值
                    if($(".meitian").is(':checked')){
                        order_limit_type=1;
                        //活动2 满多少元放券
                        order_limit =  $(".every").val();

                    }else if($(".huodong ").is(':checked')){
                        order_limit_type=2;
                        //活动2 满多少元放券
                        order_limit =  $(".during").val();
                    ...
                    $.ajax({
                        ...
                        data:{
                            ...
                            'order_limit_type':order_limit_type
